...
void ExpandKey(unsigned char key[16]) {
	for (int i = 0; i < 16; i++) {
...
unsigned char reverse_cipher[16];
...
void main(void){
	ExpandKey(key);
	Encrypt(plain, expanded_key);
	prnHex(cipher, 16);
	Decrypt(cipher, expanded_key);
	prnHex(reverse_cipher, 16);
	int res = memcmp(reverse_cipher, plain, 16);
	if(res)
		printf("fail");
	else
		printf("ok");
